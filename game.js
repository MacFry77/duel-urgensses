...
const $=id=>document.getElementById(id);let socket,state,openLobbies=[],activeGames=[],session=JSON.parse(localStorage.getItem('duel-session')||'null'),lastChatCount=0,reconnectTimer=null,connectionNumber=0,leavingRoom=false,errorTimer=null,swRegistration=null,deferredInstallPrompt=null,pushEndpoint='',pushPublicKey='',pendingSpectatorName='',podiumPresented=false,pendingMatchIntro=false;
...
function connect(){
  ...
  nextSocket.onmessage=e=>{if(attempt!==connectionNumber)return;const m=JSON.parse(e.data);if(m.type==='lobbies'){openLobbies=Array.isArray(m.lobbies)?m.lobbies:[];activeGames=Array.isArray(m.activeGames)?m.activeGames:[];renderOpenChallenges();renderLiveGames()}if(m.type==='session'){session={code:m.code,playerId:m.playerId};localStorage.setItem('duel-session',JSON.stringify(session))}if(m.type==='state'){const previousStatus=state?.status,previousPhase=state?.phase;state=m.state;if((previousStatus==='lobby'||previousPhase==='over')&&state.status==='playing'){pendingMatchIntro=true;podiumPresented=false;if($('podiumDialog').open)$('podiumDialog').close()}$('connectionBanner').classList.add('hidden');render();if(state.viewerRole==='spectator'&&pendingSpectatorName){const name=pendingSpectatorName;pendingSpectatorName='';send('renameSpectator',{name})}}if(m.type==='left')returnHome();if(m.type==='closed'||m.type==='kicked'){alert(m.message);leavingRoom=false;returnHome()}if(m.type==='joinUnavailable'){exitInvitationMode(true);showError(`${m.message} Vous pouvez ${m.canSpectate?'l’observer avec le code conservé, ou ':''}créer ou rejoindre un autre défi.`)}if(m.type==='error')showError(m.message)}
}
