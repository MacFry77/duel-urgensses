...
function connect(){
  ...
  nextSocket.onmessage=e=>{if(attempt!==connectionNumber)return;const m=JSON.parse(e.data);if(m.type==='lobbies'){openLobbies=Array.isArray(m.lobbies)?m.lobbies:[];renderOpenChallenges()}if(m.type==='session'){session={code:m.code,playerId:m.playerId};localStorage.setItem('duel-session',JSON.stringify(session))}if(m.type==='state'){state=m.state;$('connectionBanner').classList.add('hidden');render();if(state.viewerRole==='spectator'&&pendingSpectatorName){const name=pendingSpectatorName;pendingSpectatorName='';send('renameSpectator',{name})}}if(m.type==='left')returnHome();if(m.type==='closed'||m.type==='kicked'){alert(m.message);leavingRoom=false;returnHome()}if(m.type==='error')showError(m.message)}
}
