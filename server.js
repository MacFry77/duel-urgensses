...
function scoreRound(room){room.players.forEach(p=>{const hasBid=p.bid!==null&&p.bid!==undefined&&p.bid!=='',bid=Number(p.bid),tricks=Number(p.tricks),round=Math.max(1,Math.min(8,Number(room.round)||1)),scoreBefore=Number(p.score)||0,exact=hasBid&&Number.isInteger(bid)&&Number.isInteger(tricks)&&bid===tricks;p.score=scoreBefore;p.totalBid=(Number(p.totalBid)||0)+(Number.isInteger(bid)&&hasBid?bid:0);p.boldestBid=Math.max(Number(p.boldestBid)||0,Number.isInteger(bid)&&hasBid?bid:0);if(exact){p.exactRounds=(Number(p.exactRounds)||0)+1;if(bid===0)p.zeroSuccesses=(Number(p.zeroSuccesses)||0)+1;p.score+=bid===0?round*10:tricks*20}else p.missedRounds=(Number(p.missedRounds)||0)+1;(p.roundHistory||(p.roundHistory=[])).push({round,bid:Number.isInteger(bid)&&hasBid?bid:null,tricks:Number.isInteger(tricks)?tricks:0,exact,scoreBefore,points:p.score-scoreBefore,scoreAfter:p.score});});}
